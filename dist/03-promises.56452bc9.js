...
function submit(e) {
  e.preventDefault();
  var delay = Number(formRef.elements.delay.value);
  var step = Number(formRef.elements.step.value);
  var amount = Number(formRef.elements.amount.value);
  ...
  for (var position = 1; position <= amount; position++) {
    createPromise(position, delay).then(function (_ref) {
      var position = _ref.position,
          delay = _ref.delay;
      setTimeout(function () {
        _notiflixNotifyAio.Notify.success("\u2705 Fulfilled promise ".concat(position, " in ").concat(delay, "ms"));
      }, delay);
    }).catch(function (_ref2) {
      var position = _ref2.position,
          delay = _ref2.delay;

      _notiflixNotifyAio.Notify.failure("\u274C Rejected promise ".concat(position, " in ").concat(delay, "ms"));
    });
    delay += step;
  }
}
